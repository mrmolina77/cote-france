            ...
            <div class="overflow-x-auto">
                <table class="items-center bg-transparent w-full border-collapse text-sm">
                    <thead><tr>
                        ...
                        <th wire:click="order('subtotal')" class="cursor-pointer px-3 bg-blueGray-50 text-blueGray-500 border py-3 text-xs uppercase">Subtotal</th><th wire:click="order('descuento')" class="cursor-pointer px-3 bg-blueGray-50 text-blueGray-500 border py-3 text-xs uppercase">Descuento</th><th wire:click="order('total')" class="cursor-pointer px-3 bg-blueGray-50 text-blueGray-500 border py-3 text-xs uppercase">Total</th><th wire:click="order('saldo_pendiente')" class="cursor-pointer px-3 bg-blueGray-50 text-blueGray-500 border py-3 text-xs uppercase">Saldo</th>
                    ...
                    <tbody>@forelse($cargos as $cargo)
                        ...
                        <tr wire:key="cargo-{{ $cargo->cargo_id }}">
                            ...
                            <td class="px-3 py-3 border-t whitespace-nowrap">{{ $cargo->moneda }} ${{ $cargo->subtotal }}</td><td class="px-3 py-3 border-t whitespace-nowrap text-gray-600">{{ $cargo->moneda }} ${{ $cargo->descuento }}</td><td class="px-3 py-3 border-t whitespace-nowrap">{{ $cargo->moneda }} ${{ $cargo->total }}</td><td class="px-3 py-3 border-t whitespace-nowrap">{{ $cargo->moneda }} ${{ $cargo->saldo_pendiente }}</td>
                    ...
                    @empty<tr><td colspan="13" class="px-4 py-8 text-center text-gray-500">No se encontraron cargos.</td></tr>@endforelse</tbody>
                </table>
            </div>
