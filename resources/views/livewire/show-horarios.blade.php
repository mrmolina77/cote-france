    ...
    <div class="w-full px-0" wire:ignore.self>
        @if ($semanal)
            ...
            <div @class([
                ...
                @foreach ( $dias as $dia )
                    ...
                    @endphp
                    <div class="border-r border-gray-200 p-0.25 sticky top-0 bg-gray-50 z-10 day-header-group" style="grid-column: span {{ count($profesores) }};" data-start-index="{{ $startColumnIndex }}" data-span="{{ count($profesores) }}">
                        ...
                        <div class="grid day-header-grid" style="grid-template-columns: repeat({{ count($profesores) }}, 1fr);">
                            ...
                            @endphp
                            <div class="w-full items-center justify-center p-0.25 collapsible-header" data-column-index="{{ $columnMapping[$dateString][$profesor->profesores_id] ?? '' }}" data-column-key="{{ $columnKey }}">
                    ...
                    @foreach ($dias as $dia)
                        @foreach ($profesores as $profesor)
                            ...
                            @if ($horarioItem)
                                ...
                                @endphp
                                <div class="h-full p-0.25 text-center {{$cellgrupo}}"
                            ...
                            @elseif ($isBlocked)
                                <div class="h-full p-0.25 text-center">
                            ...
                            @else
                                ...
                                @if($grupoDetalle)
                                    @php
                                        ...
                                        $columnIndex = $columnMapping[$currentDateString][$profesor->profesores_id] ?? null;
                                    @endphp
                                    <div class="h-full p-0.25 text-center grupo-cell"
                                        data-id="0"
                                        data-dia="{{$currentDateString}}"
        ...
        @else
            ...
            <div @class([
                ...
                <div class="grid min-w-max border border-gray-200 rounded-lg overflow-hidden" id="horarios-table" style="display: grid; grid-template-columns: auto repeat({{ count($profesores) }}, minmax(3.84rem, 1fr));">
                {{-- Professor Headers --}}
                  <div class="border-r border-gray-200 px-1 py-0.5 w-16 sticky top-0 bg-gray-50 z-10 flex items-center justify-center font-sans font-semibold text-sm">{{ __('Hours') }}</div>
                @foreach ( $profesores as $profesor )
